...
const resolveMapTile = (type, x, y, mapData, setCurrentMap) => {
  ...
  switch (type) {
    ...
    case 'C':
      return <Coin key={key} position={[x, 0.5, y]} mapData={mapData} setCurrentMap={setCurrentMap} type={type} />;
    case 'E':
...
const Level01 = () => {
  ...
  return (
    <>
      ...
      <Player />
      {memoizedMapData}
      <Plane position={[0, 0, 0]} rotation={[-Math.PI / 2, 0, 0]} colour="#7E370C" />
      <Plane position={[0, 2, 0]} rotation={[+Math.PI / 2, 0, 0]} colour="#7E370C" />
      <ambientLight intensity={0.1} />
      <FlatObject position={[10, 0.5, 20]} texture={orb} />
      <pointLight position={[10, 1.1, 10]} intensity={30} castShadow={true} color="blue" />
      <FlatObject position={[20, 0.5, 20]} texture={orb} />
      <spotLight position={[10, 10, 10]} angle={0.5} intensity={1} castShadow={true} penumbra={1} />
      <pointLight position={[20, 1.1, 20]} intensity={0.2} castShadow={true} color="red" />
      <Stats />
    </>
  );
};
